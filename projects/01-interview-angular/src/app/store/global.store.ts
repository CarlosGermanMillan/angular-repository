...
import {addEntity, removeEntity, updateAllEntities, updateEntity, withEntities} from '@ngrx/signals/entities';
    ...
    withMethods((store, characterService = inject(CharacterService)) => ({
        ...
        async addCharacter(character: Omit<Character, "id">){
            try{
                ...
                // patchState(store,addEntity({id: new Date().getTime(), ...character}))
        ...
        async updateCharacter(character: Character){
            try{
                ...
                // patchState(store, updateEntity({id:character.id,changes:{...character}}))
                patchState(store, ({characters}) => ({
                    characters: [...characters.map((char) => char.id === character.id ? {...char, ...character} : char)]
                }))
            }catch(error){}
        },
    })),
